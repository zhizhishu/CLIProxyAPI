...
func TestNormalizeClaudeUpstreamErrorStatus_ServiceUnavailable429Becomes503(t *testing.T) {
	tests := []struct {
		name string
		body []byte
	}{
		...
		{
			name: "json message",
			body: []byte(`{"type":"error","error":{"type":"api_error","message":"Service Unavailable"}}`),
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := normalizeClaudeUpstreamErrorStatus(http.StatusTooManyRequests, tt.body)
			if got != http.StatusServiceUnavailable {
				t.Fatalf("normalizeClaudeUpstreamErrorStatus() = %d, want %d", got, http.StatusServiceUnavailable)
			}
		})
	}
}

func TestNormalizeClaudeUpstreamErrorStatus_Quota429Stays429(t *testing.T) {
	got := normalizeClaudeUpstreamErrorStatus(http.StatusTooManyRequests, []byte(`{"error":{"message":"quota exhausted"}}`))
	if got != http.StatusTooManyRequests {
		t.Fatalf("normalizeClaudeUpstreamErrorStatus() = %d, want %d", got, http.StatusTooManyRequests)
	}
}

func TestClaudeExecutor_ExecuteServiceUnavailable429Returns503(t *testing.T) {
	executor, auth := newClaudeServiceUnavailable429Executor(t)
	payload := []byte(`{"messages":[{"role":"user","content":[{"type":"text","text":"hi"}]}]}`)
	...
	if err == nil {
		t.Fatal("Execute() error = nil, want 503")
	}
	assertStatusErr(t, err, http.StatusServiceUnavailable)
}

func TestClaudeExecutor_ExecuteStreamServiceUnavailable429Returns503(t *testing.T) {
	executor, auth := newClaudeServiceUnavailable429Executor(t)
	payload := []byte(`{"messages":[{"role":"user","content":[{"type":"text","text":"hi"}]}]}`)
	...
	if err == nil {
		t.Fatal("ExecuteStream() error = nil, want 503")
	}
	assertStatusErr(t, err, http.StatusServiceUnavailable)
}

func TestClaudeExecutor_CountTokensServiceUnavailable429Returns503(t *testing.T) {
	executor, auth := newClaudeServiceUnavailable429Executor(t)
	payload := []byte(`{"messages":[{"role":"user","content":[{"type":"text","text":"hi"}]}]}`)
	...
	if err == nil {
		t.Fatal("CountTokens() error = nil, want 503")
	}
	assertStatusErr(t, err, http.StatusServiceUnavailable)
}

func newClaudeServiceUnavailable429Executor(t *testing.T) (*ClaudeExecutor, *cliproxyauth.Auth) {
	t.Helper()
